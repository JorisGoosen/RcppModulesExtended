...
#ifndef Rcpp_Module_Extended_CLASS_h
#define Rcpp_Module_Extended_CLASS_h
...
class class_ : public class_Base
{
  public:
    ...
    typedef std::map<std::string, named_args_method_class *> map_named_args_method;
    ...
    class_(const char *name_, const char *doc = 0) : class_Base(name_, doc),
                                                     vec_methods(),
                                                     named_args_methods(),
                                                     properties(),
                                                     finalizer_pointer(0),
...
#include <RcppModulesExtended/module/Module_generated_class_constructor.h>
#include <RcppModulesExtended/module/Module_generated_class_factory.h>
  ...
  public:
    ...
    map_named_args convert_pairlist_to_named_args(SEXP pairlist_start)
    {
        map_named_args out;
        Rcpp::RObject it = pairlist_start;
        int unnamedArgs = 0;
        while(!it.isNULL())
        {
            std::string argName;
    ...
    SEXP invoke_pairlist(SEXP method_xp, SEXP object, SEXP args)
    {
        BEGIN_RCPP

        named_args_method_class *pm = reinterpret_cast<named_args_method_class *>(EXTPTR_PTR(method_xp));
        method_class *m = pm->method;

        if (m->is_void())
        {
            m->operator()(XP(object), convert_pairlist_to_named_args(args));
            return Rcpp::List::create(true);
        }
        else
        {
            return Rcpp::List::create(false, m->operator()(XP(object), convert_pairlist_to_named_args(args)));
        }
        END_RCPP
    }

    SEXP invoke_void_pairlist(SEXP method_xp, SEXP object, SEXP args)
    {
        BEGIN_RCPP
    ...
    SEXP invoke_notvoid_pairlist(SEXP method_xp, SEXP object, SEXP args)
    {
        BEGIN_RCPP
        ...
        return m->operator()(XP(object), convert_pairlist_to_named_args(args));
        END_RCPP
    }
    ...
    self &AddNamedArgsMethod(const char *name_, method_class *m, const char *docstring = 0)
    {
        RCPP_DEBUG_MODULE_1("AddNamedArgsMethod( %s, method_class* m, const char* docstring = 0", name_)
        self *ptr = get_instance();
        ptr->named_args_methods[name_] = new named_args_method_class(m, docstring);

        return *this;
    }

    
#include <RcppModulesExtended/module/Module_method_named_args.h>
#include <RcppModulesExtended/module/Module_generated_method.h>
#include <RcppModulesExtended/module/Module_generated_Pointer_method.h>

    bool has_method(const std::string &m)
    {

        return vec_methods.find(m) != vec_methods.end() || named_args_methods.find(m) != named_args_methods.end();
    }
    ...
    Rcpp::CharacterVector method_names()
    {
        ...
        for (int i = 0; i < s; i++, ++it)
            n += (it->second)->size();
        for (typename map_named_args_method::iterator named_args_method = named_args_methods.begin(); named_args_method != named_args_methods.end(); ++named_args_method)
            n += 1;
    ...
    Rcpp::IntegerVector methods_arity()
    {
        ...
        for (typename map_named_args_method::iterator named_args_method = named_args_methods.begin(); named_args_method != named_args_methods.end(); ++named_args_method)
        {
            mnames[k] = named_args_method->first;
            res[k] = 1;
            k++;
        }
    ...
    Rcpp::LogicalVector methods_voidness()
    {
        ...
        {
            mnames[k] = named_args_method->first;
            res[k] = named_args_method->second->is_void();
            k++;
        }
        res.names() = mnames;
        return res;
    ...
    Rcpp::CharacterVector complete()
    {
        ...
        int ntotal = n + properties.size() + named_args_methods.size();
        ...
        for (typename map_named_args_method::iterator it = named_args_methods.begin(); it != named_args_methods.end(); ++it)
            out[i++] = it->first + "("; //named_args_method shouldn't contain "[", to change this the concept should be extended
    ...
    Rcpp::List getMethodsNamedArgs(const XP_Class &class_xp, std::string &buffer)
    {
        RCPP_DEBUG_MODULE("Rcpp::List getMethodsNamedArgs( const XP_Class& class_xp, std::string& buffer")
#if RCPP_DEBUG_LEVEL > 0
        Rf_PrintValue(class_xp);
#endif
        for(typename map_named_args_method::iterator named = named_args_methods.begin(); named != named_args_methods.end(); ++named)
            if(vec_methods.count(named->first) > 0)
                throw std::runtime_error("Do not use a named-args-method with the same name as a normal method");
        int n = named_args_methods.size();
        Rcpp::CharacterVector mnames(n);
        Rcpp::List res(n);

        int i = 0;
        for(typename map_named_args_method::iterator named = named_args_methods.begin(); named != named_args_methods.end(); ++named, i++)
        {
            mnames[i] = named->first;
            res[i] = S4_CppOverloadedMethods<Class>(named->second, class_xp, named->first.c_str(), buffer);
        }
...
#include <RcppModulesExtended/module/Module_Field.h>

#include <RcppModulesExtended/module/Module_Add_Property.h>
    ...
    class_() : class_Base(), vec_methods(), named_args_methods(), properties(), specials(0), constructors(), factories(){};
  ...
  public:
    ...
    self &derives(const char *parent)
    {
        ...
        // importing methods_named
        typename parent_class_::map_named_args_method::iterator parent_named_args_methods_iterator  = parent_class_pointer->named_args_methods.begin();
        typename parent_class_::map_named_args_method::iterator parent_named_args_methods_end       = parent_class_pointer->named_args_methods.end();
        for (; parent_named_args_methods_iterator != parent_named_args_methods_end; parent_named_args_methods_iterator++)
        {
            method_name = parent_named_args_methods_iterator->first;

            typedef typename parent_class_::named_args_method_class parent_named_args_method_class;
            parent_named_args_method_class *p_method = parent_named_args_methods_iterator->second;
            parent_method_class *parent_method = p_method->method;

                CppMethod<Class> *method = new CppInheritedMethod<Class, PARENT>(parent_method);

                AddNamedArgsMethod(method_name.c_str(), method, p_method->docstring.c_str());
        }
